...
from ..models.contribution import BatchContributionCreate, Contribution, ContributionCreate
...
async def get_contribution(db: Client, contribution_id: str) -> Contribution | None:
...
async def record_contributions_batch(
    ...
    items_json = json.dumps(
        [{"account_id": item.account_id, "amount": float(item.amount)} for item in data.items]
    )

    result = db.rpc(
        "record_contributions_batch",
        {
            "p_data": items_json,
            "p_contribution_type": data.contribution_type,
            "p_period_year": data.period_year,

...
from ..dependencies import _format_cop
...
async def validate_loan_request(db: Client, data: LoanCreate) -> tuple[bool, str]:
    ...
    if data.backing_type == "individual":
        if account["balance"] < data.amount_requested:
            return False, (f"Insufficient balance. Available: {_format_cop(account['balance'])}")
        ...
        if data.amount_requested > max_collective:
            return False, (f"Exceeds 20% cap. Max: {_format_cop(max_collective)}")

    elif data.backing_type == "group":
        guarantor_total = sum(g.guaranteed_amount for g in data.guarantors)
        fund_portion = data.amount_requested - guarantor_total
        if fund_portion > 0:
            fund = db.table("fund_summary").select("*").eq("id", 1).execute()
            if not fund.data:
                return False, "Fund summary not found"
            available = float(fund.data[0]["available_for_lending"])
            max_collective = available * 0.20
            if fund_portion > max_collective:
                return False, (
                    f"FONAFAHE portion ({_format_cop(fund_portion)}) exceeds 20% cap. "
                    f"Max: {_format_cop(max_collective)}"
                )
...
async def get_available_balance(db: Client, account_id: str) -> float:
    ...
    guarantees = (
        ...
        .select("guaranteed_amount, loan_id")
        .eq("guarantor_account_id", account_id)
        .eq("status", "accepted")
        .execute()
    )
...
async def validate_group_backing(db: Client, loan_id: str) -> tuple[bool, str]:
    ...
    guarantors = db.table("loan_guarantors").select("*").eq("loan_id", loan_id).execute()
    ...
    if total < float(loan["amount_requested"]):
        return False, (
            ...
            f"Need: {_format_cop(loan['amount_requested'])}, Have: {_format_cop(total)}"
        )
    ...
    for g in guarantors.data:
        ...
        if available < float(g["guaranteed_amount"]):
            return False, (
                ...
                f"available balance ({_format_cop(available)})"
            )
...
async def approve_loan(
    db: Client, loan_id: str, approved_by: str, amount: float | None = None
) -> Loan:
    loan = db.table("loans").select("*").eq("id", loan_id).execute()
...
async def respond_guarantor(db: Client, guarantor_id: str, accept: bool) -> LoanGuarantor:

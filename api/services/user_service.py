...
from ..models.user import MergeRequest, MergeRequestDetail, User


async def enroll_national_id(
    db: Client, user_id: str, national_id: str
) -> tuple[str, MergeRequest | None]:
    """
    ...
    if existing.data:
        ...
        db.table("users").update({"national_id": national_id}).eq("id", user_id).execute()
...
async def get_pending_merge_requests(db: Client) -> list[MergeRequestDetail]:
    ...
    for row in result.data:
        req_user = (
            ...
            .eq("id", row["requesting_user_id"])
            .execute()
        )
        tgt_user = (
            db.table("users").select("full_name, email").eq("id", row["target_user_id"]).execute()
        )
...
async def get_user_merge_status(db: Client, user_id: str) -> str | None:
